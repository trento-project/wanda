...
if [ "$SKIP_GENERATION" = false ]; then
    ...
    # Remove the x.y.z from the components, just use VX_ (ex: V1_) and remove unversioned prefix
    sed -i -E 's/"[0-9.]+-unversioned_([^"]+)"/"\1"/g; s/#\/components\/schemas\/[0-9.]+-unversioned_([^"]+)/#\/components\/schemas\/\1/g; s/"[0-9.]+-(v[0-9]+)_/"\1_/g; s/#\/components\/schemas\/[0-9.]+-(v[0-9]+)_/#\/components\/schemas\/\1_/g' "$MERGED_OPENAPI_FILE"
