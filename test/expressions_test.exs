defmodule ExpressionsTest do
  ...
  test "is empty" do
    ...
    abb =
      Rhai.eval(
        ...
        facts.cluster_nodes.all(|node| node.uname in facts.hosts)
    ...
    # assert {:ok, ^is_empty} = Rhai.eval("!env.provider.is_empty()", context)
  ...
  describe "check 1.1.9 DA114A Corosync has at least 2 rings configured" do
    ...
    test "should support what really comes from the agent" do
      context = %{
        "facts" => %{
          "corosync_nodes" => [
            %{
      ...
      assert {:ok, true} =
               Rhai.eval(
                 ~s"""
                 facts.corosync_nodes.all(|node| node.keys().filter(|prop| prop.starts_with("ring")).len() == values.expected_corosync_rings_per_node)
                 """,
                 context
               )
    end
